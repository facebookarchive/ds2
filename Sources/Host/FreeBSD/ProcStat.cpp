...
bool ProcStat::GetProcessMap(pid_t pid, std::vector<ds2::MemoryRegionInfo> &entries) {
    struct procstat *pstat;
    struct kinfo_proc *kip;
...
bool ProcStat::EnumerateAuxiliaryVector(pid_t pid, std::function<void(
                ELFSupport::AuxiliaryVectorEntry const &)> const &cb) {
...
void ProcStat::EnumerateProcesses(bool allUsers, UserId const &uid, std::function<void(pid_t pid, uid_t uid)> const &cb) {
    struct procstat *pstat;
    struct kinfo_proc *kip;
    unsigned int count, i;
...
void ProcStat::EnumerateThreads(pid_t pid, std::function<void(pid_t tid)> const &cb) {
    struct procstat *pstat;
    struct kinfo_proc *kip;
    unsigned int count, i;
...
std::string ProcStat::GetExecutablePath(
        ProcessId pid) {
    return "unknown";
}
}
