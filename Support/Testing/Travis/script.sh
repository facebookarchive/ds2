...
# CentOS uses different compiler names than Ubuntu, so we can only use the toolchain files on Ubuntu
# In addition, clang-3.7 is also not available for CentOS
